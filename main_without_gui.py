...
def demo_0():
    # given values
    N = 5 # N is (int) a number of items, just the length of the list
    V = 4 # V is (float) a deadline, items with important indexes must be processed by this value of time
    K, L = 2, 4 # K and L are the two important indexes
    I = [K, L] # a list of important indexes
    U = 0 # a time of (float) the start
    
    # lists (down below) are the lists filled with the time of processing an each item
    # (each element is a time of processing current item with that index)
    ...
    #  so there is a list to find: a valid list with the minimum sum of all elements
